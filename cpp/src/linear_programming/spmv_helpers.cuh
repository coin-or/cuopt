...
template <typename i_t, typename f_t, i_t block_dim, typename view_t>
  ...
  if (block_count > 0) {
    ...
    if (!dry_run) {
      lb_spmv_block_kernel<i_t, f_t, block_dim>
        <<<block_count, block_dim, 0, block_stream>>>(item_id_beg, view, input, output);
    }
  }
}
...
{
  if (view.nnz < 10000) {
    spmv_per_block<i_t, f_t, 32>(streams, view, input, output, bin_offsets, 32, 32, dry_run);
    spmv_per_block<i_t, f_t, 64>(streams, view, input, output, bin_offsets, 64, 64, dry_run);
    spmv_per_block<i_t, f_t, 128>(streams, view, input, output, bin_offsets, 128, 128, dry_run);
    spmv_per_block<i_t, f_t, 256>(streams, view, input, output, bin_offsets, 256, 256, dry_run);
  } else {
    //[1024, heavy_degree_cutoff/2] -> 1024 block size
    spmv_per_block<i_t, f_t, 1024>(
      streams, view, input, output, bin_offsets, 1024, heavy_degree_cutoff / 2, dry_run);
    //[512, 512] -> 128 block size
    spmv_per_block<i_t, f_t, 128>(streams, view, input, output, bin_offsets, 128, 512, dry_run);
  }
}

template <typename i_t, typename f_t, i_t threads_per_constraint, typename view_t>
    ...
    if (!dry_run) {
      lb_spmv_sub_warp_kernel<i_t, f_t, block_dim, threads_per_constraint>
        <<<block_count, block_dim, 0, sub_warp_thread>>>(
          item_id_beg, item_id_end, view, input, output);
    }
  }
}
                   ...
                   bool dry_run)
{
  spmv_sub_warp<i_t, f_t, threads_per_constraint>(
    streams, view, input, output, degree, degree, bin_offsets, dry_run);
}
  ...
  if (block_count != 0) {
    ...
    if (!dry_run) {
      lb_spmv_sub_warp_kernel<i_t, f_t, block_dim><<<block_count, block_dim, 0, sub_warp_stream>>>(
        view, input, output, make_span(warp_item_offsets), make_span(warp_item_id_offsets));
    }
  }
}
                   ...
                   bool dry_run = false)
{
  if (view.nnz < 10000) {
    ...
    spmv_sub_warp<i_t, f_t, 16>(streams, view, input, output, 16, bin_offsets, dry_run);
    spmv_sub_warp<i_t, f_t, 8>(streams, view, input, output, 8, bin_offsets, dry_run);
    spmv_sub_warp<i_t, f_t, 4>(streams, view, input, output, 4, bin_offsets, dry_run);
    spmv_sub_warp<i_t, f_t, 2>(streams, view, input, output, 2, bin_offsets, dry_run);
    spmv_sub_warp<i_t, f_t, 1>(streams, view, input, output, 1, bin_offsets, dry_run);
  } else {
    if (is_sub_warp_single_bin) {
      // std::cout<<"spmv_sub_warp single_bin\n";
      spmv_sub_warp<i_t, f_t, 16>(streams, view, input, output, 64, bin_offsets, dry_run);
      spmv_sub_warp<i_t, f_t, 8>(streams, view, input, output, 32, bin_offsets, dry_run);
      spmv_sub_warp<i_t, f_t, 4>(streams, view, input, output, 16, bin_offsets, dry_run);
      spmv_sub_warp<i_t, f_t, 2>(streams, view, input, output, 8, bin_offsets, dry_run);
      spmv_sub_warp<i_t, f_t, 1>(streams, view, input, output, 1, 4, bin_offsets, dry_run);
    } else {
