...
class spmv_functor_t {
 public:
  ...
  spmv_functor_t(problem_t<i_t, f_t>& problem,
                 ...
                 bool debug = false);
  ~spmv_functor_t();
  spmv_functor_t() = delete;
  void setup_lb_problem(problem_t<i_t, f_t>& problem, bool debug = false);
  void setup_lb_meta();
  spmv_view_t get_A_view();
  spmv_view_t get_AT_view();
  template <typename OutputTransformer>
  void call_Ax_graph(raft::device_span<f_t> input, OutputTransformer t, bool dry_run = false);
  template <typename OutputTransformer>
  void call_ATy_graph(raft::device_span<f_t> input, OutputTransformer t, bool dry_run = false);
...
template <typename i_t, typename f_t>
template <typename OutputTransformer>
void spmv_functor_t<i_t, f_t>::call_Ax_graph(raft::device_span<f_t> input,
                                             OutputTransformer t,
                                             bool dry_run)
{
  ...
  spmv_per_block<i_t, f_t>(streams, view, input, t, cnst_bin_offsets, heavy_degree_cutoff, dry_run);
...
template <typename i_t, typename f_t>
template <typename OutputTransformer>
void spmv_functor_t<i_t, f_t>::call_ATy_graph(raft::device_span<f_t> input,
                                              OutputTransformer t,
                                              bool dry_run)
{
  ...
  spmv_per_block<i_t, f_t>(streams, view, input, t, vars_bin_offsets, heavy_degree_cutoff, dry_run);
  spmv_sub_warp<i_t, f_t>(streams,
                          view,
                          input,
                          t,
